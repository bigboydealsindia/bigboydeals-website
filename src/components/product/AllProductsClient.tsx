...
export function AllProductsClient({
  ...
  const prices = initialProducts.map((p) => Number(p.sellingPrice) || 0);
  ...
  const getMainCatCount = (id: number) =>
    initialProducts.filter((p) => p.mainCategoryId === id).length;
  const getSubCatCount = (id: number) =>
    initialProducts.filter((p) => p.subCategoryId === id).length;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedSubCat) {
      ...
      result = result.filter((p) => p.mainCategoryId === selectedMainCat);
    }
  ...
  const FilterContent = () => (
    <div className="space-y-6 pb-8">
      <div className="space-y-4">
        <h3 className="font-bold text-lg tracking-tight">Categories</h3>
        <Separator />
        <div className="space-y-2">
          <div
            className={`text-sm cursor-pointer hover:text-primary transition-colors ${!selectedMainCat ? "font-bold text-primary" : "text-muted-foreground"}`}
            onClick={() => {
          ...
          {mainCategories.map((mainCat) => {
            const count = getMainCatCount(mainCat.id);
            if (count === 0) return null;
            const isExpanded = selectedMainCat === mainCat.id;
            const subCategories = categories.filter(
              (c) => c.parentId === mainCat.id,
            );

            return (
              <div key={mainCat.id} className="space-y-2 pt-1">
                <div
                  className={`text-sm cursor-pointer flex items-center justify-between hover:text-primary transition-colors ${isExpanded ? "font-bold text-foreground" : "text-muted-foreground"}`}
                  onClick={() => {
                    setSelectedMainCat(isExpanded ? null : mainCat.id);
                    setSelectedSubCat(null);
                  }}
                >
                ...
                <AnimatePresence>
                  {isExpanded && (
                    <motion.div
                      ...
                      className="pl-4 space-y-2 overflow-hidden border-l-2 border-secondary/50 ml-1"
                    >
                      {subCategories.map((subCat) => {
                        const subCount = getSubCatCount(subCat.id);
                        if (subCount === 0) return null;
                        return (
                          <div
                            key={subCat.id}
                            className={`text-sm cursor-pointer flex items-center justify-between hover:text-primary transition-colors ${selectedSubCat === subCat.id ? "font-bold text-primary" : "text-muted-foreground"}`}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            size="sm"
            className="h-9 rounded-md px-4 font-bold bg-primary hover:bg-primary/90"
          >
            Go
          </Button>
        </div>
            ...
            >
              <AnimatePresence>
                {filteredProducts.map((product) => {
                  const brand = initialBrands.find(
