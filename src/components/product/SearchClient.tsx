...
export function SearchClient({ query, products, brands }: SearchClientProps) {
  ...
  // Absolute Price boundaries from DB results
  ...
  const handleWishlistClick = async (
    ...
    if (!res.success) {
      toggleItem(productId); // Revert UI
  ...
  const FilterSidebarContent = () => (
    <div className="space-y-6 pb-8">
      {/* Search Metadata block */}
      <div className="space-y-2 bg-secondary/10 p-4 rounded-xl border border-border">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest">
          Searching for
        </span>
        <h3 className="font-bold text-xl tracking-tight text-foreground truncate">
          "{query}"
        </h3>
      </div>
    ...
    <div className="max-w-[1400px] mx-auto w-full px-4 md:px-8 py-6 min-h-screen">
      ...
      <div className="flex flex-col md:flex-row gap-8 items-start">
        ...
        <div className="flex-1 w-full">
          ...
          {products.length === 0 ? (
            <div className="py-24 text-center flex flex-col items-center justify-center border-2 border-dashed border-border rounded-xl bg-secondary/5 mt-4">
              ...
              <p className="text-muted-foreground text-sm max-w-sm">
                ...
                checking your spelling or use more general terms.
              </p>
              <Button
                onClick={() => router.push("/all-products")}
                className="mt-8 rounded-md font-bold px-8"
              >
                Browse All Products
              </Button>
            </div>
          ) : filteredProducts.length === 0 ? (
            // State: No Filtered Results
            <div className="py-20 text-center flex flex-col items-center justify-center border-2 border-dashed border-border rounded-xl bg-secondary/5">
              ...
              <p className="text-muted-foreground text-sm max-w-sm">
                We found items for "{query}", but none of them match your
                ...
                {filteredProducts.map((product) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <div className="relative w-full aspect-[4/5] bg-secondary/10 rounded-md overflow-hidden mb-2.5 border border-border/50">
                          <Image
        ...
        <AlertDialogContent className="rounded-[var(--radius)] border-border">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base text-muted-foreground mt-2 leading-relaxed">
              Looks like you're exploring as a guest. Please log in first to
